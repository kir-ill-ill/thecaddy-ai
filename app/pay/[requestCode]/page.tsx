...
export default function PaymentPage({ params }: { params: Promise<{ requestCode: string }> }) {
  ...
  if (error || !paymentRequest) {
    return (
      <div className="min-h-screen bg-sand/30 flex items-center justify-center px-4">
    ...
    return (
      <div className="min-h-screen bg-sand/30 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2 font-serif">Already Paid!</h2>
  ...
  return (
    <div className="min-h-screen bg-sand/30 flex items-center justify-center px-4 py-12">
      {/* Toast notification */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 px-4 py-3 rounded-lg shadow-lg text-sm font-medium bg-red-600 text-white animate-fade-in">
          {toast.message}
        </div>
      )}
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-forest to-forest/90 px-6 py-8 text-white text-center">
            <div className="bg-gold/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
              <DollarSign className="w-8 h-8 text-gold" />
            </div>
            <p className="text-sand/80 text-sm mb-1">{paymentRequest.trip_name}</p>
            <h1 className="text-2xl font-bold mb-2 font-serif">{paymentRequest.fund.name}</h1>
            <p className="text-4xl font-bold text-gold">{paymentRequest.amount_formatted}</p>
          </div>

          {/* Description */}
          {paymentRequest.fund.description && (
            <div className="px-6 py-4 bg-sand/30 border-b border-forest/10">
          ...
          <form onSubmit={handlePayment} className="p-6 space-y-4">
            ...
            <button
              ...
              className="w-full px-6 py-4 bg-gold text-forest rounded-lg font-semibold hover:bg-gold/90 transition disabled:opacity-50 flex items-center justify-center gap-3"
            >
              {isProcessing ? (
                <>
        ...
        <div className="mt-6 text-center">
          ...
          <div className="flex justify-center gap-2">
            <div className="bg-white px-3 py-2 rounded border border-forest/10 text-xs font-medium text-gray-600">
              Visa
            </div>
            <div className="bg-white px-3 py-2 rounded border border-forest/10 text-xs font-medium text-gray-600">
              Mastercard
